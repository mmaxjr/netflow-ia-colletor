...
| Arquivo | O que é |
|---|---|
| `monitor.py` | O coletor + analisador. Roda para sempre, escutando NetFlow e chamando a IA periodicamente. |
| `setup_netflow_debian.sh` | Script que configura um servidor Debian do zero: instala o exportador (`softflowd`), o Python, e sobe `monitor.py` como serviço systemd. |
| `netflow_reports.log` | Gerado em tempo de execução — histórico de todos os relatórios e análises da IA. |
...
```
 ...
 ┌──────────────────┐   NetFlow v5   ┌──────────────────────────────┐
 │  interface eth0   │ ─── UDP 2055 ─▶│ softflowd  →  monitor.py     │
...
2. **`monitor.py`** escuta essa porta, decodifica os pacotes e acumula os
...
## Explicando o código (`monitor.py`)
...
4. Grava `monitor.py` em `/opt/netflow-ai/monitor.py`
...
para `127.0.0.1:2055` — o `monitor.py` processa normalmente e você já
